...
def validate_execution_context(path: str | Path) -> list[str]:
    ...
    if (
        data.get("context_type") in {"sandbox", "restricted"}
        and data.get("production_access") is True
    ):
        errors.append(
...
def validate_action_preflight(path: str | Path) -> list[str]:
    data = load_yaml(path)
    errors = _validate_required_fields(
        data, ACTION_PREFLIGHT_REQUIRED_FIELDS, "action_preflight"
    )
    ...
    for field in [
        ...
        if field in data and data[field] not in {
            "passed",
            "failed",
            "not_required",
            "unknown",
...
def validate_capability_verification(path: str | Path) -> list[str]:
    data = load_yaml(path)
    errors = _validate_required_fields(
        data, CAPABILITY_VERIFICATION_REQUIRED_FIELDS, "capability_verification"
    )
...
def validate_capability_registration(path: str | Path) -> list[str]:
    ...
    for field in ["id", "name", "description", "repository", "version"]:
        if field in data and (
            ...
            errors.append(
                f"capability_registration: Field must be a non-empty string: {field}"
            )
    ...
    if not isinstance(interfaces, dict):
        ...
        for section in ["inputs", "outputs"]:
            ...
            if not isinstance(values, list):
                errors.append(
                    f"capability_registration: Interface section must be a list: {section}"
                )
                continue
            for index, interface in enumerate(values):
                ...
                if "required" in interface and not isinstance(
                    interface["required"], bool
                ):
    ...
    if not isinstance(metadata, dict):
        ...
        for field in ["owner", "compliance_level", "last_audit"]:
            if not isinstance(metadata.get(field), str) or not metadata[field].strip():
                errors.append(
                    f"capability_registration: Missing or invalid metadata field: {field}"
                )
...
def validate_protected_resource_boundary(path: str | Path) -> list[str]:
    data = load_yaml(path)
    errors = _validate_required_fields(
        data, PROTECTED_RESOURCE_BOUNDARY_REQUIRED_FIELDS, "protected_resource_boundary"
    )
    if "resource_scope" in data and not isinstance(data["resource_scope"], dict):
        errors.append(
            "protected_resource_boundary: Field must be an object: resource_scope"
        )
    ...
    if "audit_required" in data and not isinstance(data["audit_required"], bool):
        errors.append(
            "protected_resource_boundary: Field must be a boolean: audit_required"
        )
...
def validate_evidence_claim_binding(path: str | Path) -> list[str]:
    data = load_yaml(path)
    errors = _validate_required_fields(
        data, EVIDENCE_CLAIM_BINDING_REQUIRED_FIELDS, "evidence_claim_binding"
    )
    if data.get("freshness_status") not in {"fresh", "stale", "unknown"}:
        errors.append("evidence_claim_binding: Invalid freshness_status")
    if data.get("validator_result") not in {
        "passed",
        "failed",
        "insufficient_evidence",
    }:
        errors.append("evidence_claim_binding: Invalid validator_result")
    if "same_object_verified" in data and not isinstance(
        data["same_object_verified"], bool
    ):
        errors.append(
            "evidence_claim_binding: Field must be a boolean: same_object_verified"
        )
    if data.get("validator_result") == "passed":
        if data.get("freshness_status") != "fresh":
            errors.append(
                "evidence_claim_binding: Passed claim requires fresh evidence"
            )
...
def validate_failure_circuit_breaker(path: str | Path) -> list[str]:
    data = load_yaml(path)
    errors = _validate_required_fields(
        data, FAILURE_CIRCUIT_BREAKER_REQUIRED_FIELDS, "failure_circuit_breaker"
    )
    if data.get("state") not in {"closed", "open", "half_open"}:
        errors.append("failure_circuit_breaker: Invalid state")
    if data.get("next_action") not in {
        "continue",
        "stop",
        "retry_probe",
        "require_review",
    ...
    for field in ["threshold", "current_count"]:
        if field in data and (not isinstance(data[field], int) or data[field] < 0):
            errors.append(
                f"failure_circuit_breaker: Field must be a non-negative integer: {field}"
            )
    if (
...
def validate_agent(path: str | Path) -> list[str]:
    ...
    if data.get("type", "agent") != "agent":
        errors.append("Field must equal agent: type")

    if data.get("enforcement_mode", "strict") not in {"strict", "permissive"}:
        errors.append("Invalid enforcement_mode")
...
def validate_runtime_binding(data: dict) -> list[str]:
    ...
    if "target_resource_scope" in data and not isinstance(
        data["target_resource_scope"], dict
    ):
...
def validate_pre_dispatch_authorization_check(data: dict) -> list[str]:
    ...
    if "matched_approved_binding" in data and not isinstance(
        data["matched_approved_binding"], bool
    ):
        errors.append(
            "pre_dispatch_authorization_check: Field must be a boolean: matched_approved_binding"
        )
    ...
    if "observed_target_resource_scope" in data and not isinstance(
        data["observed_target_resource_scope"], dict
    ):
        errors.append(
            "pre_dispatch_authorization_check: Field must be an object: observed_target_resource_scope"
        )
...
def validate_execution_receipt(data: dict) -> list[str]:
    ...
    if (
        "authorization_result" in data
        and data["authorization_result"] not in AUTHORIZATION_RESULTS
    ):
        errors.append("execution_receipt: Invalid authorization_result")

    if (
        "side_effect_state" in data
        and data["side_effect_state"] not in SIDE_EFFECT_STATES
    ):
    ...
    if "matched_approved_binding" in data and not isinstance(
        ...
        errors.append(
            "execution_receipt: Field must be a boolean: matched_approved_binding"
        )
    ...
    if "target_resource_scope" in data and not isinstance(
        ...
        errors.append(
            "execution_receipt: Field must be an object: target_resource_scope"
        )

    if (
        data.get("authorization_result") in {"denied", "expired", "reapproval_required"}
        and data.get("side_effect_state") != "blocked_before_side_effect"
    ):
        errors.append(
            "execution_receipt: Negative authorization results must block before side effect"
        )

    if (
        data.get("authorization_result") == "allowed"
        and data.get("matched_approved_binding") is not True
    ):
        errors.append(
            "execution_receipt: Allowed receipts must match an approved binding"
        )
...
def validate_runtime_example(path: str | Path) -> list[str]:
    ...
    if "pre_dispatch_authorization_check" in data:
        errors.extend(
            validate_pre_dispatch_authorization_check(
                data["pre_dispatch_authorization_check"]
            )
        )
...
def validate_production_action_boundary_example(path: str | Path) -> list[str]:
    ...
    if isinstance(boundary, dict):
        ...
        if "target_resource_scope" in boundary and not isinstance(
            boundary["target_resource_scope"], dict
        ):
            errors.append("boundary: Field must be an object: target_resource_scope")

        if (
            "approval_level" in boundary
            and boundary["approval_level"] not in PRODUCTION_APPROVAL_LEVELS
        ):
...
def _validate_string_list_values(
    data: dict, fields: list[str], prefix: str
) -> list[str]:
    errors: list[str] = []
    ...
    for field in fields:
        ...
        for index, item in enumerate(value):
            if not isinstance(item, str) or not item.strip():
                errors.append(
                    f"{prefix}: List item must be a non-empty string: {field}[{index}]"
                )
...
def _validate_lifecycle_mode_object(data: dict, prefix: str) -> list[str]:
    ...
    for target in (
        data.get("valid_handoff_targets", [])
        if isinstance(data.get("valid_handoff_targets"), list)
        else []
    ):
...
def validate_lifecycle_map(path: str | Path) -> list[str]:
    ...
    if missing_modes:
        errors.append(
            "lifecycle_map: Missing lifecycle modes: " + ", ".join(missing_modes)
        )
...
def validate_lifecycle_receipt(path: str | Path) -> list[str]:
    data = load_yaml(path)
    errors = _validate_required_fields(
        data, REQUIRED_LIFECYCLE_RECEIPT_FIELDS, "lifecycle_receipt"
    )
...
def validate_lifecycle_schema(path: str | Path) -> list[str]:
    data = load_yaml(path)
    errors = _validate_required_fields(
        data, REQUIRED_LIFECYCLE_SCHEMA_FIELDS, "lifecycle_schema"
    )
