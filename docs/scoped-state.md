...
authentication. Signed scoped message transport or an external identity
provider is required when sender authenticity matters. See
[Authenticated scoped messages](message-transport.md).

This is a scoped persistence primitive, not a complete workspace protocol. It
does not implement network delivery, handoff authorization, key rotation,
retention/garbage collection, or external attestation.
